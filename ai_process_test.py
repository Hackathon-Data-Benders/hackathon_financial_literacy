...
def get_plan_recommendations(QA_res):
    """
    Use OpenAI to get course recommendations based on user responses.
    ...
    """
    template = """
    You only answer in the list<json> format where key of the json will have the value title and description .
    Respective values for title should be the type of Financial Plans , values of description should describe the plan in 1 statement.
    Based on the answers for the below questions,
    what should be the suitable financial plans for young adults , age between 20-30 years
    This is in the perspective of the user for your context.DO NOT include ```json or ``` at the start and end of content
    {QA_res}
    """
    prompt = PromptTemplate.from_template(template)
    llm = ChatOpenAI(model="gpt-4o", default_headers={"Authorization" : os.environ["BODHI_LLM_GATEWAY_TOKEN"]})
    llm_chain = prompt | llm 
    response = llm_chain.invoke({"QA_res":QA_res})
    print(response.content)
    # Evaluate the response to convert it into a list
    recommended_plans = eval(response.content)
    print("Recommended plans:", recommended_plans)
    return recommended_plans


def generate_plan_paragraph(title):
    template="Write an extremely detailed paragraph on {title}. Make it very understandable and well formatted.It should be ideal for 20-30 years young adult"
    prompt = PromptTemplate.from_template(template)
    llm = ChatOpenAI(model="gpt-4o", default_headers={"Authorization" : os.environ["BODHI_LLM_GATEWAY_TOKEN"]})
    llm_chain = prompt | llm 
    overview = llm_chain.invoke({"title": title})
